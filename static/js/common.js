...
var common = (function() {
  ...
  function mimeTypeForTool(tool) {
    ...
    if (isHostToolchain(tool)) {
      ...
      mimetype = 'application/x-nacl'; //FIXME: replaced from 'application/x-pnacl' to work for tizen didn't bother to correctly do it. 
    }
  ...
  function createNaClModule(name, tool, path, width, height, attrs) {
    console.log('Parameters:', name, tool, path, width, height, attrs);
    ...
    if (attrs) {
      for (var key in attrs) {
        ...
        console.log('After setting ' + key + ':', moduleEl);
      }
    }
    ...
    console.log('%c[createNaClModule, common.js]', 'color: gray;', "moduleEl: " + moduleEl.outerHTML);
